...
async function main() {
  ...
  const user = await prisma.user.upsert({
    where: { email: 'demo@Scribbles.io' },
    update: { hashedPassword, emailVerified: new Date() },
    create: {
      id: 'user_1',
      name: 'Demo User',
      email: 'demo@Scribbles.io',
      hashedPassword,
  ...
  const collections = [
    ...
    { id: 'col_toolkit',  name: 'Dev Toolkit',        description: 'DevOps, terminal commands, and design resources', isFavorite: false },
  ]
